...
import "./styles/littlebook.css"
import dottie from "../vendor/dottie.js"
import debug from "../../graveyard/vendor/debug.js"
...
class WarningMap<K, T> extends Map<K, T> {
	label: string
...
type PatternList = Array<Pattern>

/**
 * Creates a matchmaker for a map of URLs to values.
 * @param map The map to match against.
 * @param query Optional query parameter to use for matching.
 * @returns The matchmaker object with methods to add/remove patterns and match URLs.
 */
function createMatchmaker(
	map: WarningMap<string, any>,
	query?: string
): object {
	return {
		patterns: [] as PatternList,
		...
		matchName(url: URL | string, defaultTo?: string) {
