	...
	switch (starttype) {
		...
		case tokTypes._switch:
			...
			while (!this.closes(tokTypes.braceR, blockIndent, line, true)) {
				if (
					this.tok.type === tokTypes._case ||
					this.tok.type === tokTypes._default
				) {
...
lp$1.parseClassElement = function () {
	...
	if (!keyName && ecmaVersion >= 8 && this.eatContextual("async")) {
		if (
			(this.isClassElementNameStart() || this.toks.type === tokTypes.star) &&
	...
	if (
		ecmaVersion < 13 ||
		this.toks.type === tokTypes.parenL ||
		kind !== "method" ||
		isGenerator ||
		isAsync
	) {
...
lp$1.parseImportSpecifiers = function () {
	...
	if (this.tok.type === tokTypes.star) {
		...
		while (
			...
			if (this.eat(tokTypes.star)) {
				elt$1.local = this.eatContextual("as")
	...
	for (;;) {
		...
		if (
			(optional &&
				...
				this.tok.type !== tokTypes.bracketL &&
				this.tok.type !== tokTypes.backQuote) ||
...
lp.parsePropertyAccessor = function () {
	...
	if (this.tok.type === tokTypes.privateId) {
